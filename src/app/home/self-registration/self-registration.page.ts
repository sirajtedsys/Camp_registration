...
import { SelfReg } from 'src/class/selfreg';
...
export class SelfRegistrationPage implements OnInit {
  ...
  async submit(){
    ...
    // this.sr.DeviceId = null
    // this.sr.
    this.patser.UpdateDailyTokenDetails(this.sr).subscribe((data:any)=>{
      loading.dismiss()
      console.log(data);
  ...
  async GetDefaultnationality(){
    ...
    this.patser.GetDefaultNationalityAsync().subscribe((data:any)=>{
      ...
      if(data)
      {
        ...
        // this.opdatalist=data
        // this.NationalityList = data
        this.sr.NationalityId =data.TerritoryId
  ...
  async GetDefaultPatienttype(){
    ...
    this.patser.GetDefaultPatientTypeAsync().subscribe((data:any)=>{
      ...
      if(data)
      {
        ...
        // this.opdatalist=data
        // this.NationalityList = data
        this.sr.PatiTypeId =data.TypeId
  ...
  async VerifyMobilenumber(): Promise<boolean> {
    ...
    try {
      const data: any = await this.patser.VerifymobileNumber(this.sr.Mobile).toPromise();
      loading.dismiss();
      if(data.length>0)
      {
  ...
  async formvalidation() {
    try {
      // if (this.pr.SaltId !== "" && this.pr.SaltId !== null) {
        ...
        //   {
          if (this.sr.Patient !== "" && this.sr.Patient !== null) {
            if (this.sr.PatiTypeId !== "" && this.sr.PatiTypeId !== null) {
              // if (this.pr.PatiPlaceId !== "" && this.pr.PatiPlaceId !== null) {
                if (this.sr.Mobile !== "" && this.sr.Mobile !== null) {
                  if (this.sr.Sex  !== "" && this.sr.Sex !== null) {
                    if (this.sr.Dob !== undefined && this.sr.Dob !== null && this.sr.Dob!="") {
                      // if(this.CheckLocationAmndatory())
                      // {

                        // this.sr.AgeYear = this.
  ...
  async IdcardType(){
    ...
    this.patser.GetIdentityCards().subscribe((data:any)=>{
      loading.dismiss()
      if(data)
      {
        console.log(data,'GetIdentityCards');
        // this.opdatalist=data
...
calculateDOB() {
  ...
  this.sr.AgeYear=this.Years
  // Ensure DOB is valid (e.g., if the month and day subtractions go negative, adjust)
  this.sr.Dob = birthDate.toISOString().split('T')[0]; // Format to 'YYYY-MM-DD'
}
